...
int readi(struct inode *ip, char *dst, uint off, uint n) {
  ...
  uint start_byte = off - size* BSIZE;

  for (tot = 0; tot < n; tot += m, off += m, dst += m, start_byte += m) {

    if(start_byte < data->nblocks * BSIZE){
    bp = bread(ip->dev, data->startblkno + start_byte / BSIZE);
    m = min(n - tot, BSIZE - start_byte % BSIZE);
    memmove(dst, bp->data + start_byte % BSIZE, m);
    brelse(bp);
    }else{

      data++;
      m = 0;
      start_byte = off - start_byte;
